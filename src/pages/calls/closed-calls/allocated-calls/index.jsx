...
const AllocatedCallsCC = ({ data }) => {
  ...
  const getQuery = () => {
    ...
    if (filters?.callScore?.value && filters?.callScore?.value !== "") {
      query += `call_score=${filters?.callScore?.value}`;
    }
    if (filters?.onTimeReview?.value && filters?.onTimeReview?.value !== "") {
      query += `on_time_review=${filters?.onTimeReview?.value}`;
    }
    if (filters?.delayTime?.value && filters?.delayTime?.value !== "") {
      query += `delay_time=${filters?.delayTime?.value}`;
    }
    return query;
  };
